...
def build(map_path, train_color = StationType.NORMAL):
  ...
  if train_color == StationType.GREEN:
    removable_stations = [key for key,value in map["stations"].items() if value==StationType.RED.value]
  elif train_color == StationType.RED:
    removable_stations = [key for key,value in map["stations"].items() if value==StationType.GREEN.value]
...
def _colapse_node(node, connections):
  ...
  nodes = set(list_node for pair in remove_conn for list_node in pair if list_node != node)
